  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl flex items-center justify-between flex-col md:flex-row  ">
        ...
        <div className="relative mt-8 md:mt-0 w-full max-w-88 md:max-w-104">
          ...
          {floatingLogos.map((logo) => (
            <div
              key={logo.label}
              className={`absolute ${logo.position}  z-10 flex h-20 w-20 items-center justify-center  transition-transform duration-300 hover:scale-105`}
            >
              {logo.icon}
            </div>
          ))}
